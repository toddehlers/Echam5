...
!---wiso-code
   ...
   IF (nwiso .GT. 0) THEN
    ...
    iwiso=nwiso
    IF (iwiso .EQ. 1) iwiso=2  !Attention: This is a first, very crude quick&dirty solution to allow nwiso=1 in WISO namelist
    
    CALL add (g3b,'wisows',      wisows      ,code=51             ,longname='soil wetness - water isotopes'                        &
                                                                                   ,units='m'       ,klev=iwiso)
    CALL add (g3b,'wisosn',      wisosn      ,code=52             ,longname='snow depth - water isotopes'                          &
                                                                                  ,units='m'        ,klev=iwiso)
    CALL add (g3b,'wisoaprl',    wisoaprl    ,code=53,laccu=.TRUE.,longname='large scale precipitation - water isotopes'           &
                                                                          ,bits=24 ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisoaprc',    wisoaprc    ,code=54,laccu=.TRUE.,longname='conv.  precipitation - water isotopes'                &
                                                                          ,bits=24 ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisoaprs',    wisoaprs    ,code=55,laccu=.TRUE.,longname='snow fall - water isotopes'                           &
                                                                          ,bits=24 ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisoxivi',    wisoxivi    ,code=56,laccu=.TRUE.,longname='vertically integrated cloud ice - water isotopes'     &
                                                                                   ,units='kg/m**2' ,klev=iwiso)
    CALL add (g3b,'wisorunoff',  wisorunoff  ,code=57,laccu=.TRUE.,longname='surface runoff and drainage - water isotopes'         & 
                                                                          ,bits=24 ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisodrain',   wisodrain   ,code=58,laccu=.TRUE.,longname='drainage - water isotopes'                            & 
                                                                          ,bits=24 ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisoevap',    wisoevap    ,code=59,laccu=.TRUE.,longname='evaporation - water isotopes'                         &
                                                                          ,bits=24 ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisoevapiac', wisoevapiac ,code=60,laccu=.TRUE.,longname='evaporation over ice - water isotopes'                &
                                                                                   ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisoevapwac', wisoevapwac ,code=61,laccu=.TRUE.,longname='evaporation over water - water isotopes'              &
                                                                                   ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisoevaplac', wisoevaplac ,code=62,laccu=.TRUE.,longname='evaporation over land - water isotopes'               &
                                                                                   ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisowl',      wisowl      ,code=63             ,longname='skin reservoir content - water isotopes'              &
                                                                                   ,units='m'       ,klev=iwiso)
    CALL add (g3b,'wisogld',     wisogld     ,code=64             ,longname='glacier depth - water isotopes'                       & 
                                                                                   ,units='m'       ,klev=iwiso)
    CALL add (g3b,'wisorogl',    wisorogl    ,code=65,laccu=.TRUE.,lpost=.FALSE.,longname='glacier runoff - water isotopes'        & 
                                                                                   ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisosnmel',   wisosnmel   ,code=66,laccu=.TRUE.,longname='snow melt - water isotopes'                           & 
                                                                                   ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisoapmegl',  wisoapmegl  ,code=67,laccu=.TRUE.,longname='P-E over land ice - water isotopes'                   & 
                                                                          ,bits=24 ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisosnacl',   wisosnacl   ,code=68,laccu=.TRUE.,longname='snow accumulation over land - water isotopes'         & 
                                                                                   ,units='kg/m**2s',klev=iwiso)
    CALL add (g3b,'wisoqvi',     wisoqvi     ,code=69,laccu=.TRUE.,longname='vertically integrated water vapor - water isotopes'   &
                                                                                   ,units='kg/m**2' ,klev=iwiso)
    CALL add (g3b,'wisoxlvi',    wisoxlvi    ,code=70,laccu=.TRUE.,longname='vertically integrated cloud water - water isotopes'   &
                                                                                   ,units='kg/m**2' ,klev=iwiso)
    CALL add (g3b,'wisosnc',     wisosnc     ,code=71             ,longname='snow depth at the canopy - water isotopes'            & 
                                                                                   ,units='m'       ,klev=iwiso)

    CALL add (g3b,'wisosw_d',    wisosw_d    ,code=72             ,longname='delta of surface seawater - water isotopes'           & 
                                                                                   ,units='permil',klev=iwiso)
...
!---wiso-code
    ...
    IF (nwiso .GT. 0) THEN
     CALL add (g3b,'wisoxtec',wisoxtec,lpost=.FALSE. ,ktrac=nwiso)
    END IF
