...
! - 1D
         , pwisoevap                                                   &
         , pwisoevapl, pwisoevapw, pwisoevapi                          &
         , pwisoevaplac,pwisoevapwac,pwisoevapiac                      &
         , pwisosn,    pwisows,    pwisowl                             &                   
         , pwisosw_d                                                   &
! - 1D within physics only.
         , pwisoevapot                                                 &
! - Tendencies
...
!  REAL(dp):: zmult5, zplmax, zplmin, zq2m, zqddif, zqdp, zqklevi         !---wiso-code: change dimension of zqklevi
  REAL(dp):: zmult5, zplmax, zplmin, zq2m, zqddif, zqdp, zqklevi(kbdim)
!  REAL(dp):: zqklevl, zqklevw, zqlwi1, zqlwi2, zqmitte, zqnlev           !---wiso-code: change dimension of zqklevl & zqklevw
  REAL(dp):: zqklevl(kbdim), zqklevw(kbdim), zqlwi1, zqlwi2, zqmitte, zqnlev
...
!---wiso-code
  ...
  REAL(dp):: zwisodisqi,zwisodisql,zwisodisqw
  REAL(dp):: zwisodqdt
  REAL(dp):: zwisodximdt, zwisodxlmdt
!NEW  REAL(dp):: zwisoqklevi, zwisoqklevl, zwisoqklevw, zwisoqnlev
  REAL(dp):: zwisoqklevi(kbdim,kwiso), zwisoqklevl(kbdim,kwiso), zwisoqklevw(kbdim,kwiso), zwisoqnlev, zwisoqnlev_land
  REAL(dp):: zwisozqs
...
!---wiso-code
  ...
  REAL(dp)::                                                           &
      ...
      ,pwisoevapl(kbdim,kwiso),pwisoevapw(kbdim,kwiso)                 &
      ,pwisoevapi(kbdim,kwiso)                                         &
      ,pwisoevaplac(kbdim,kwiso),pwisoevapwac(kbdim,kwiso)             &
      ,pwisoevapiac(kbdim,kwiso)                                       &
      ,pwisosn(kbdim,kwiso),pwisowl(kbdim,kwiso)                       &
      ,pwisows(kbdim,kwiso)                                            &
      ,pwisosw_d(kbdim,kwiso)                                          &
      ,pwisoevapot(kbdim,kwiso)                                        &
...
!---wiso-code
  ...
  REAL(dp)::                                                           &
          zwisowet(kbdim)
  REAL(dp)::                                                           &
          zwisocsat(kbdim,kwiso),zwisocair(kbdim,kwiso)                &  
         ,zwisoqdif(kbdim,klev,kwiso)
  REAL(dp)::                                                           &
          zwisoxldif(kbdim,klev,kwiso),zwisoxidif(kbdim,klev,kwiso)

  REAL(dp)::                                                           &
          zwisoqsl(kbdim,kwiso),zwisoqsw(kbdim,kwiso)                  &
         ,zwisoqsi(kbdim,kwiso)                                        &
         ,zwisoqhfll(kbdim,kwiso),zwisoqhflw(kbdim,kwiso)              &
         ,zwisoqhfli(kbdim,kwiso)

  REAL(dp)::                                                           &
          zwet_tmp(kbdim),                   &   ! dummy field to store a preliminary value of zwet
          zdeltaws(kbdim,kwiso),             &   ! isotopic ratio (delta-value)
          zdeltawl(kbdim,kwiso),             &   ! in skin/soil/snow water reservoir
          zdeltasn(kbdim,kwiso),             &   !
          zwisokin(kbdim,kwiso),             &   ! kinetic fractionation factor at air/sea interface
          zwspeed, zwspeedmin,               &   ! wind speed and minimum wind speed for kin. fraction
          zwisosurfmin, zwisoroundoff,       &
          ztsw_tmp,ztsi_tmp,                 &
          zwisofracl,zwisofracw, zwisofraci, &
          zsatval,zocean,zice,zwisosec
...
!---wiso-code
  ...
  REAL(dp)::    zwisoeqnl(kbdim,kwiso)    ! richtmyer-morton-coefficients
  REAL(dp)::    zwisofqnl(kbdim,kwiso)    !
  REAL(dp)::    zwisoeqnw(kbdim,kwiso)    !
  REAL(dp)::    zwisofqnw(kbdim,kwiso)    ! for moisture - water isotopes
  REAL(dp)::    zwisoeqni(kbdim,kwiso)    !
  REAL(dp)::    zwisofqni(kbdim,kwiso)    !
  REAL(dp)::    zwisoqslnew(kbdim,kwiso)  ! new surf. sat. spec. humidity - water isotopes
...
!          fractionation over water
           ztsw_tmp=ptsw(jl)
           zwisofracw=exp(talphal1(jt)/(ztsw_tmp**2._dp)+talphal2(jt)/ztsw_tmp+talphal3(jt))
!          "corrections" of ocean surface isotope values by prescribed seasurface delta values 
           zocean=toce(jt)* (1._dp+(pwisosw_d(jl,jt)/1000._dp))
...
!
           ztsi_tmp=ptsi(jl)
           zwisofraci=exp(talphas1(jt)/(ztsi_tmp**2._dp)+talphas2(jt)/ztsi_tmp+talphas3(jt))
!          effective fractionation over ice if necessary (isotopes only)
           IF (nwisotyp(jt).ne.1.and.ztsi_tmp.lt.tmelt) THEN
              zsatval=tsatbase - tsatfac*(ztsi_tmp-tmelt)
              zwisofraci=zwisofraci*(zsatval/(1._dp+zwisofraci*(zsatval-1._dp)*tdifrel(jt)))
           ENDIF
!          "corrections" of seaice surface isotope values by prescribed seasurface delta values 
           zice=toce(jt)* (1._dp+(pwisosw_d(jl,jt)/1000._dp))
...
!
 ...
 330 CONTINUE
     DO 331 jl=1,kproma
!---wiso-code
        zwet_tmp(jl)=zwet(jl)          !---wiso-code: storage of a temporary result of the evapotranspiration
!---wiso-code-end
...
! (see also: g. hoffmann, dissertation, p.29ff)
          ...
          IF (pws(jl) .GT. zwslim*pwsmx(jl)) THEN
              zwisowet(jl)= pcvs(jl)*zdeltasn(jl,jt)                        &
                      + (1._dp-pcvs(jl))*(pcvw(jl)*zdeltawl(jl,jt)      &
                      + (1._dp-pcvw(jl))*zdeltaws(jl,jt)                & 
                      / (1._dp+zchl(jl)*SQRT(zdu2(jl))*zwet_tmp(jl)))
          ELSE
             zwisowet(jl) = pcvs(jl)*zdeltasn(jl,jt)                       &
                      + (1._dp-pcvs(jl))* pcvw(jl)*zdeltawl(jl,jt) 
          ENDIF
          ...
          zwisocsat(jl,jt)=pcvs(jl)*zdeltasn(jl,jt)+(1._dp-pcvs(jl))                     &
                         *(pcvw(jl)*zdeltawl(jl,jt)+(1._dp-pcvw(jl))*zdeltaws(jl,jt)     &
                         *MERGE(0._dp,zhum(jl),lo))
          zwisocair(jl,jt)=pcvs(jl)*zdeltasn(jl,jt)+(1._dp-pcvs(jl))                     &
                         *(pcvw(jl)*zdeltawl(jl,jt)+(1._dp-pcvw(jl))*zdeltaws(jl,jt)     &
                         *MERGE(0._dp,1._dp,lo))

          zwisosec=MERGE(pcvs(jl)*zdeltasn(jl,jt)+((1._dp-pcvs(jl))*(pcvw(jl)*zdeltawl(jl,jt)  &
                  + (1._dp-pcvw(jl))*zdeltaws(jl,jt))),1._dp,lpland(jl))

          lo=pqm1(jl,klev).GT.zqsl(jl)

          zwisocsat(jl,jt)=MERGE(zwisosec,zwisocsat(jl,jt),lo)
          zwisocair(jl,jt)=MERGE(zwisosec,zwisocair(jl,jt),lo)

          zwisocsat(jl,jt)=pvgrat(jl)*zwisowet(jl)+(1._dp-pvgrat(jl))*zwisocsat(jl,jt)
          zwisocair(jl,jt)=pvgrat(jl)*zwisowet(jl)+(1._dp-pvgrat(jl))*zwisocair(jl,jt)

          zwisocsat(jl,jt)=MERGE(zwisocsat(jl,jt)*tnat(jt),zwisocsat(jl,jt),lpland(jl))
          zwisocair(jl,jt)=MERGE(zwisocair(jl,jt)*tnat(jt),zwisocair(jl,jt),lpland(jl))
...
!           no kin. fractionation over land surface
            zwisokin(jl,jt)=MERGE(1._dp,1._dp-zwisokin(jl,jt),lpland(jl))
         END DO
      END DO
!---wiso-code-end
...
!
     DO 532 jk=itopp1,klevm1
        DO 531 jl=1,kproma
           ...
           zdisv=1._dp/(1._dp+zfav*(1._dp-zebsv(jl,jk-1))                 &
                                                    +zcfv(jl,jk)*zqdp)
...
!       land   +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
        zwisodisql=1._dp/(1._dp+zfac*(1._dp-zebsh(jl,klevm1)))                                  
        zwisoeqnl(jl,jt)=zwisodisql*zcfhl(jl)*zqdp
        zwisofqnl(jl,jt)=zwisodisql*(zwisoqdif(jl,klev,jt)+zfac*zwisoqdif(jl,klevm1,jt))*ztpfac1
!
!       water   +++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
        zwisodisqw=1._dp/(1._dp+zfac*(1._dp-zebsh(jl,klevm1))              &
...
!       ice   +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
        zwisodisqi=1._dp/(1._dp+zfac*(1._dp-zebsh(jl,klevm1))              &
                                         +zcfhi(jl)*zqdp)
...
!    Land surface saturation specific humidity (qsat=zqslnew) - water isotopes
...
!
        ...
        zqklevl(jl)=zeqnl(jl)*zqslnew(jl)+zfqnl(jl)        !---wiso-code: changed dimension of zqklevl
!
!    water   +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
        ztklevw=zetnw(jl)*zcptw(jl)+zftnw(jl)
        zqklevw(jl)=zeqnw(jl)*zqsw(jl)+zfqnw(jl)           !---wiso-code: changed dimension of zqklevw
!
!    ice   +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
        ztklevi=zetni(jl)*zcpti(jl)+zftni(jl)
        zqklevi(jl)=zeqni(jl)*zqsi(jl)+zfqni(jl)           !---wiso-code: changed dimension of zqklevi
!
...
!
        ...
        zqdif(jl,klev)=(zgql*zqklevl(jl)+zgqw*zqklevw(jl)+zgqi*zqklevi(jl))/zgqsum !---wiso-code: changed dimension of zqklevl/e/i
548  END DO
!
!---wiso-code
!
...
!
        zwisoqklevl(jl,jt)=zwisoeqnl(jl,jt)                                      &
                           *(zwisocsat(jl,jt)*zwisoqslnew(jl,jt)                 &
                            -zwisocair(jl,jt)*zqdif(jl,klev)*ztpfac1)            &
                           +zwisofqnl(jl,jt)
!
!      water   +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
...
!    Surface fluxes of moisture - water isotopes
     ...
     DO jt=1,kwiso
      DO jl=1,kproma
       ...
       zcoefl=zcons15*zcfhl(jl)
       zcoefw=zcons15*zcfhw(jl)*zwisokin(jl,jt)
       zcoefi=zcons15*zcfhi(jl)
...
!
        ...
        zqnlev=zqdif(jl,klev)-ztpfac3*pqm1(jl,klev)
        zwisoqnlev_land=MERGE(zqnlev,zwisoqnlev,lpland(jl))
!
!       land   +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
        zwisozqs=ztpfac2*zwisoqslnew(jl,jt)
        zwisoqhfll(jl,jt)=zcoefl*(zwisocair(jl,jt)*zwisoqnlev_land-zwisocsat(jl,jt)*zwisozqs)
...
!
          pwisoevapot(jl,jt)=zcoefl*(zwisoqnlev_land-zwisozqs)
