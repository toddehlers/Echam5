...
!---wiso-code
  USE mo_wiso,          ONLY: nwiso, wisoq_names, wisoxl_names, wisoxi_names
!---wiso-code-end
...
!---wiso-code
  ...
  ! set print flags of water isotopes in gridpoint (spitfire transport) output stream buffer
  ! (flags are set equal to the values of the normal water fields)
  
  IF (nwiso .GT. 0) THEN
    DO jt=1,nwiso
      CALL set_stream_element_info (gl ,wisoq_names(jt)    ,lpost=lppq)    ! humidity
      CALL set_stream_element_info (gl ,wisoxl_names(jt)   ,lpost=lppxl)   ! cloud water
      CALL set_stream_element_info (gl ,wisoxi_names(jt)   ,lpost=lppxi)   ! cloud ice
    END DO
  END IF
