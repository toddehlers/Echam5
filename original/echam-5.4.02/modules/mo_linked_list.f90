...
!------------------------------------------------------------------------------
  ...
  USE mo_filename,     ONLY: NETCDF, GRIB, NETCDF64, out_filetype
  USE mo_filename,     ONLY: NONE, SZIP, out_ztype
...
!------------------------------------------------------------------------------
  ...
  TYPE (memory_info) ,INTENT(in) :: info
    ...
    WRITE(nout,'(a, i4)') '  gridID             : ',info% gridID
    WRITE(nout,'(a, i4)') '  zaxisID            : ',info% zaxisID
    WRITE(nout,*)
...
!------------------------------------------------------------------------------
  ...
  TYPE (t_stream) ,INTENT(in) :: stream ! output stream to print
    ...
    CASE (NONE)
    WRITE(nout,'(a    )') '  output compression : NONE'
    CASE (SZIP)
    WRITE(nout,'(a    )') '  output compression : SZIP'
    CASE default
    WRITE(nout,'(a    )') '  output compression : UNKNOWN'
    END SELECT
