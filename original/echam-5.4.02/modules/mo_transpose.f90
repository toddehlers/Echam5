...
!
#if defined (__crayx1) || defined (sun) || defined (NAG) || defined (__SX__) || defined (ES) || defined (__PGI)
...
MODULE mo_transpose
...
#ifndef NOMPI
                              p_communicator_a, &! communicator group a
                              p_real_dp,        &! base real(dp) data type
                              p_int,            &! base integer data type
                              p_address_kind,   &! import MPI address kind.
#endif
  ...
  !
  ! alternativ gridpoint <-> Fourier space transposition
  !
  LOGICAL, PARAMETER :: l_alt_transpose = .FALSE.
  !
  ! public routines:
  !
...
#ifndef NOMPI
  !
  ! fields for alternate transpose routines
  !
  INTEGER, ALLOCATABLE :: counts(:,:)          ! MPI_Alltoallw counts
  INTEGER, ALLOCATABLE :: displs(:,:)          ! MPI_Alltoallw displs
  INTEGER, ALLOCATABLE :: all2all_types(:,:)   ! MPI data types
  LOGICAL              :: all2all_types_initialized(2)
  !
#endif
  !
!==============================================================================
...
!TMP    INTEGER :: i, j, k, l, n
    ...
    IF (l_alt_transpose) THEN
#ifndef NOMPI       
      SELECT CASE (sign)
        CASE ( 1)
          CALL tr_gp_fs_alt (gl_dc, gp1, gp2, gp3, gp4, gp5, gp6, gp7, &
               sf3=sf3,                                                &
               zm1=zm1, zm2=zm2, zm3=zm3,                              &
               fs=fs, fs0=fs0)
        CASE (-1)
          CALL tr_fs_gp_alt (gl_dc, gp1, gp2, gp3, gp4, gp5, gp6, gp7, &
               gp8=gp8, gp9=gp9,                                       & 
...
!==============================================================================
#ifndef NOMPI
  SUBROUTINE tr_gp_fs_alt (gl_dc, gp1, gp2, gp3, gp4, gp5, gp6, gp7, &
                           sf1, sf2, sf3, zm1, zm2, zm3, fs, fs0)
    !
    ! transpose grid point space  -> Fourier space
    !
    !
    TYPE (pe_decomposed) ,INTENT(in)   :: gl_dc  (:)       ! decomposition
    ...
    REAL(dp) ,OPTIONAL ,INTENT(inout)  :: sf2    (:,:)     ! gridpoint space 2d
    REAL(dp) ,OPTIONAL ,INTENT(inout)  :: sf3    (:,:)     ! gridpoint space 2d
    REAL(dp)           ,INTENT(inout)  :: zm1    (:,:)     ! zonal mean
    REAL(dp)           ,INTENT(inout)  :: zm2    (:,:)     ! zonal mean
    REAL(dp)           ,INTENT(inout)  :: zm3    (:,:)     ! zonal mean
    REAL(dp)           ,INTENT(inout)  :: fs     (:,:,:,:) ! Fourier space
    REAL(dp)           ,INTENT(inout)  :: fs0    (:,:,:)   ! zonal mean, Four.
    !
    ! local variables
    !
    INTEGER, PARAMETER :: send_3d   = 1
    INTEGER, PARAMETER :: recv_3d   = 2
    INTEGER, PARAMETER :: send_2d   = 3
    INTEGER, PARAMETER :: recv_2d   = 4

!TMP    INTEGER :: i, j, k, l, n
    INTEGER :: i, j, k
    INTEGER :: imype         ! index of this pe
    INTEGER :: perror        ! MPI return code

    imype  = indx (p_pe, gl_dc)
    IF (gl_dc(imype)% col_1d) RETURN

    ! If used for the first time, set up all MPI_Alltoallw entries
    ! They are the same for all calls
    IF (.NOT. ALLOCATED(all2all_types) .OR. &
        .NOT. all2all_types_initialized(1) ) THEN
       CALL setup_gp_fs_parms
    END IF

    ! Make sure the target area for the fields sf1, sf2, sf3 is properly
    ! initialized
    fs(:,gl_dc(imype)% nflevp1,:,:) = 0.0

    ! If everything is set up, do the communication
    CALL MPI_Alltoallw(                                                       &
         gp1,  counts(:,send_3d), displs(:,send_3d), all2all_types(:,send_3d),&
         fs, counts(:,recv_3d), displs(:,recv_3d), all2all_types(:,recv_3d),  &
         p_communicator_a, perror)
    CALL MPI_Alltoallw(                                                       &
         zm1, counts(:,send_2d), displs(:,send_2d), all2all_types(:,send_2d), &
         fs0, counts(:,recv_2d), displs(:,recv_2d), all2all_types(:,recv_2d), &
         p_communicator_a, perror)

  CONTAINS

    SUBROUTINE setup_gp_fs_parms

    ! various parameters, in particular field sizes
    ...
    INTEGER             :: nlon          ! global number of longitudes
    INTEGER             :: nvar          ! number of variables in fft buffer
    INTEGER             :: nproma        ! inner loop length
    INTEGER             :: ngpblks       ! number of nproma blocks
    INTEGER             :: npromz        ! nproma remainder 
    INTEGER             :: nlev          ! number of levels
    INTEGER             :: nprocb        ! number of PEs in set A
    INTEGER             :: ks, ke, nk    ! vertical range of buffer
    INTEGER             :: fs_size(4)    ! shape of target field
    LOGICAL             :: lreg          ! regular lon/lat ordering

    ! automatic arrays needed to determine the indices (with respect to
    ! gl_dc) of senders and receivers
    INTEGER             :: imype_vec(gl_dc(imype)% nprocb) ! const all imype
    INTEGER             :: gl_dc_inx(gl_dc(imype)% nprocb) ! send/recv indices

    ! variables for intermediate MPI data types.
    INTEGER             :: nproma_col    ! one full column of size nproma
    INTEGER             :: npromz_col    ! the first npromz entries
    INTEGER             :: sf_type       ! all data from one sf* field
    INTEGER             :: set_of_cols   ! ngpblks-1 copies of nproma_col
    INTEGER             :: gpx_base_raw  ! nglon*nglat sized package
    INTEGER             :: gpx_base      ! same, but with extent adjusted 
    INTEGER             :: zm_base_raw   ! one zm* column
    INTEGER             :: zm_base       ! same, but with extent adjusted
    INTEGER             :: fs_col_raw    ! nglon*nglat sized package
    INTEGER             :: fs_col        ! same, but with extent adjusted

    ! intermediate variables and vectors used in construction of data types
    LOGICAL                      :: lexcess_level
    INTEGER                      :: dt_size
    INTEGER                      :: dt_blklen(10), dt_types(10)
    INTEGER(KIND=p_address_kind) :: dt_displs(10)
    INTEGER                      :: dt2_size
    INTEGER                      :: dt2_blklen(gl_dc(imype)%nglat * 2)
    INTEGER                      :: dt2_types (gl_dc(imype)%nglat * 2)
    INTEGER(KIND=p_address_kind) :: dt2_displs(gl_dc(imype)%nglat * 2)
    INTEGER(KIND=p_address_kind) :: gp1_base   ! base address of gp1
    INTEGER(KIND=p_address_kind) :: zm1_base   ! base address of zm1
    INTEGER(KIND=p_address_kind) :: fs_base    ! base address of fs
    INTEGER(KIND=p_address_kind) :: sizeofdp   ! extent of p_real_dp
    INTEGER(KIND=p_address_kind) :: lb_dp      ! lower bound of p_real_dp
    INTEGER(KIND=p_address_kind) :: sizeofcol  ! extent of nproma_col
    INTEGER(KIND=p_address_kind) :: lb_col     ! lower bound of nproma_col
    INTEGER(KIND=p_address_kind) :: sizeoffsc  ! extent of fs_col
    INTEGER(KIND=p_address_kind) :: lb_fsc     ! lower bound of fs_col

    ! step 1 * allocation of all static fields
    nprocb  = gl_dc(imype)% nprocb
    IF (.NOT. ALLOCATED(all2all_types) ) THEN
        ALLOCATE( all2all_types(1:nprocb,1:8), &
                  counts(1:nprocb,1:8),        &
                  displs(1:nprocb,1:8) )
        all2all_types_initialized = .FALSE.
    END IF

    ! step 2 * locate relevant gl_dc indices
    ! 
    ! For each sender and receiver in the call to MPI_Alltoallw in the 
    ! main program we need the corresponding index into the gl_dc
    ! field to retrieve size data. This is done by a single call to
    ! MPI_Alltoall, sending imype around. Thereafter the receive buffer
    ! containes the required information.
    imype_vec = imype
    CALL MPI_Alltoall(imype_vec,  1, p_int, &
                      gl_dc_inx,  1, p_int, &
                      p_communicator_a, perror)

    ! The following steps have to be done for each k=1,nprocb
    DO k=1, nprocb

    ! step 3 * set up counts and displacements
    !
    ! The definition of MPI_Alltoallw, as indicated in the MPI 2 document 
    ! http://www.mpi-forum.org/docs/mpi-20-html/mpi2-report.html ,
    ! has the flaw, that the displacements are given by p_int, 
    ! which is 32-bit in general. 
    ! echam5, however, is a 64-bit application. So there is a danger 
    ! of casting 64-bit addresses to 32-bit integers.
    !
    ! Therefore, the strategy is work the displacements into the
    ! MPI data type, where we can use INTEGER(KIND=MPI_ADDRESS_KIND
    ! for this purpose - which is 64-bit for a 64-bit application.
    ! Conseqently, we send and receive one item only with displacement 0.
    
       counts(k,send_3d) = 1
       displs(k,send_3d) = 0
       counts(k,recv_3d) = 1
       displs(k,recv_3d) = 0
    ! For the 2d fields it is basically only a copy operation
    ! This is implemented as sending to itself only.
       IF ( imype == gl_dc_inx(k) ) THEN
          counts(k,send_2d) = 1
          displs(k,send_2d) = 0
          counts(k,recv_2d) = 1
          displs(k,recv_2d) = 0
    ...
    ! step 4 * MPI data types for sending 3d fields
    !
    ! Each of the fields gp1(:,:,:), ... , gp7(:,:,:) is of 
    ! shape (nproma,nlev,ngpblks). The portion to be sent to another
    ! MPI task is gpx(:,ks:ke,:), 
    ! but only nglat*nglon*(ke-ks+1) p_real_dp numbers are to be received.
    ! To achieve this, we use the fact that
    !
    ! nglat*nglon = (ngpblks - 1)*nproma + npromz
    !
    ! The sender sends (ngpblks - 1) items of size nproma*sizeof(dp) 
    ! plus one item of size npromz*sizeof(dp). The receiver receives 
    ! nglon items of size nglat*sizeof(dp). This is basically, 
    ! what the reorder2 function in this module is implementing.
    ! For this reshape to work properly, the nglon and nglat indices 
    ! (or the nproma and ngpblks indices, respectivly), have to be adjacent.
    ! So the 2nd and 3rd index of the source and target field are swapped.
    ! In other words, there is a silent transposition upon sending and
    ! receiving.
    !
    ! MPI2 calls are used to avoid problems for 64 bit addressing.

       nvar    = SIZE (fs,4)-2         
       ks      = gl_dc(gl_dc_inx(k))% flevs
       ke      = gl_dc(gl_dc_inx(k))% fleve
       nk      = gl_dc(gl_dc_inx(k))% nflevp1
       nk0     = gl_dc(gl_dc_inx(k))% nflev
       nglat   = gl_dc(imype)% nglat
       nglon   = gl_dc(imype)% nglon
       ngpblks = gl_dc(imype)% ngpblks
       nlev    = gl_dc(imype)% nlev
       nproma  = gl_dc(imype)% nproma
       npromz  = gl_dc(imype)% npromz
       lreg    = gl_dc(imype)% lreg

       CALL MPI_Type_get_extent(p_real_dp, lb_dp, sizeofdp, perror)

    ! first the contiguous data types
    ! nproma_col    * one full column of size nproma
    ! npromz_col    * the first npromz entries
       CALL MPI_Type_contiguous( nproma, p_real_dp, nproma_col, perror)
       CALL MPI_Type_contiguous( npromz, p_real_dp, npromz_col, perror)
    ! fit ngpblks-1 nproma_cols together. Mind the gap of nlev-1 
    ! columns between each two entries of nproma_col.
       CALL MPI_Type_vector( ngpblks-1, 1, nlev, nproma_col,             &
                             set_of_cols, perror )
    ! Now add the incomplete column at the end
       dt_blklen(1) = 1
       dt_blklen(2) = 1
       dt_displs(1) = 0
       dt_displs(2) = (ngpblks-1)*nlev*nproma*sizeofdp
       dt_types(1)  = set_of_cols
       dt_types(2)  = npromz_col
       CALL MPI_Type_create_struct(2, dt_blklen, dt_displs, dt_types,     &
                                 gpx_base_raw, perror)
    ! Next we have to adjust the extent. We want to handle this
    ! monster type as if it were a single copy of nproma_col.
    ! This is where the "silent transpose" happens.
       CALL MPI_Type_get_extent(nproma_col, lb_col, sizeofcol, perror)
     
       CALL MPI_Type_create_resized(gpx_base_raw, lb_col, sizeofcol,      &
                                    gpx_base, perror)
    ! To provide for the case that the sf3 fields are present
    ! a data type is set up to catch one field with one item.
       CALL MPI_Type_contiguous(nglon*nglat, p_real_dp, sf_type, perror)
    ! We only need this extra data type if there is an excess level.
       IF (ke == nlev + 1) THEN
           lexcess_level = .TRUE.
           nk = nk - 1
           ke = ke - 1
       ELSE
           lexcess_level = .FALSE.
       END IF
    ! Now piece the various gpx_base fields together
       CALL MPI_Get_address(gp1,         gp1_base,     perror)
       j = 1
       dt_blklen(j) = nk
       CALL MPI_Get_address(gp1(1,ks,1), dt_displs(j), perror)
       dt_displs(j) = dt_displs(j) - gp1_base
       dt_types(j)  = gpx_base
       j = j + 1
       IF ( lexcess_level .AND. PRESENT(sf1) ) THEN
           dt_blklen(j) = 1
           CALL MPI_Get_address(sf1, dt_displs(j), perror)
           dt_displs(j) = dt_displs(j) - gp1_base
           dt_types(j)  = sf_type
           j = j + 1
       END IF
       dt_blklen(j) = nk
       CALL MPI_Get_address(gp2(1,ks,1), dt_displs(j), perror)
       dt_displs(j) = dt_displs(j) - gp1_base
       dt_types(j)  = gpx_base
       j = j + 1
       IF ( lexcess_level .AND. PRESENT(sf2) ) THEN
           dt_blklen(j) = 1
           CALL MPI_Get_address(sf2, dt_displs(j), perror)
           dt_displs(j) = dt_displs(j) - gp1_base
           dt_types(j)  = sf_type
           j = j + 1
       END IF
       dt_blklen(j) = nk
       CALL MPI_Get_address(gp3(1,ks,1), dt_displs(j), perror)
       dt_displs(j) = dt_displs(j) - gp1_base
       dt_types(j)  = gpx_base
       j = j + 1
       IF ( lexcess_level .AND. PRESENT(sf3) ) THEN
           dt_blklen(j) = 1
           CALL MPI_Get_address(sf3, dt_displs(j), perror)
           dt_displs(j) = dt_displs(j) - gp1_base
           dt_types(j)  = sf_type
           j = j + 1
       END IF
       CALL MPI_Get_address(gp4(1,ks,1), dt_displs(j),   perror)
       CALL MPI_Get_address(gp5(1,ks,1), dt_displs(j+1), perror)
       CALL MPI_Get_address(gp6(1,ks,1), dt_displs(j+2), perror)
       CALL MPI_Get_address(gp7(1,ks,1), dt_displs(j+3), perror)
       dt_size = j + 3
       DO i=j, dt_size
          dt_blklen(i) = nk
          dt_displs(i) = dt_displs(i) - gp1_base
          dt_types(i)  = gpx_base
       END DO
       CALL MPI_Type_create_struct(dt_size, dt_blklen, dt_displs, dt_types,&
                                   all2all_types(k,send_3d), perror)
    ! Commit the result
       CALL MPI_Type_commit(all2all_types(k,send_3d), perror)

    ! step 5 * MPI data types for sending 2d fields
    !
    ! This data stream just contains contiguous blocks of real numbers
    ! with block size nk0.
       CALL MPI_Type_contiguous( nk0, p_real_dp, zm_base_raw, perror)
    ! Again, the extent is adjusted to the size of one column of zm*.
       CALL MPI_Type_get_extent(zm_base_raw, lb_col, sizeofcol, perror)
     
       CALL MPI_Type_create_resized(zm_base_raw, lb_col, &
                                    nlev*sizeofdp, &
                                    zm_base, perror)
       CALL MPI_Get_address(zm1,       zm1_base,     perror)
       CALL MPI_Get_address(zm1(ks,1), dt_displs(1), perror)
       CALL MPI_Get_address(zm2(ks,1), dt_displs(2), perror)
       CALL MPI_Get_address(zm3(ks,1), dt_displs(3), perror)
       DO i=1,3
          dt_blklen(i) = nglat
          dt_displs(i) = dt_displs(i) - zm1_base
          dt_types(i)  = zm_base
       END DO
       CALL MPI_Type_create_struct(3, dt_blklen, dt_displs, dt_types,  &
                                   all2all_types(k,send_2d), perror)
    ! Commit the result
       CALL MPI_Type_commit(all2all_types(k,send_2d), perror)

    ! step 6 * MPI data types for receiving 3d fields
    !
    ! For the MPI data type on the receiver side, the roles of imype
    ! and gl_dc_inx(k) have to be swapped.
         
       ks      = gl_dc(imype)% flevs
       ke      = gl_dc(imype)% fleve
       nk      = gl_dc(imype)% nflevp1
       nk0     = gl_dc(imype)% nflev
       nglat   = gl_dc(gl_dc_inx(k))% nglat
       nglon   = gl_dc(gl_dc_inx(k))% nglon
       glons   = gl_dc(gl_dc_inx(k))% glons
       glone   = gl_dc(gl_dc_inx(k))% glone
       nlon    = gl_dc(gl_dc_inx(k))% nlon
       nglh    = gl_dc(gl_dc_inx(k))% nglh
       nlev    = gl_dc(gl_dc_inx(k))% nlev
       fs_size = SHAPE(fs)

    ! For each k, the data stream from the sender consists of multiple
    ! packets of size nglon*nglat. On the sender side,
    ! the packet is represented by the intermediate data type 
    ! gpx_base. The corresponding packet on the receiver side is fs_col.
    ! These packets have to be placed into
    ! fs(<some range>,i,:nglat,j), where i,j are different for each packet.
    ! This is being complicated by <some range> varying over 1..nglat.
    !
    ! For 1..nglat/2, the range is given by glons(1)..glone(1)
       DO i=1,nglat/2
          dt2_blklen(i) = glone(1) - glons(1) + 1
          dt2_displs(i) = (glons(1) - 1)*sizeofdp                   &
                          + (i-1)*fs_size(1)*fs_size(2)*sizeofdp
          dt2_types(i)  = p_real_dp
       END DO
    ! For nglat/2+1..nglat, we have to compare glone(2) to glons(2).
       IF (glone(2)>glons(2)) THEN
    ! This is the simple case. There is just one range glons(2)..glone(2)
          dt2_size = nglat
          DO i=nglat/2+1,dt2_size
             dt2_blklen(i) = glone(2) - glons(2) + 1
             dt2_displs(i) = (glons(2) - 1)*sizeofdp                &
                             + (i-1)*fs_size(1)*fs_size(2)*sizeofdp
             dt2_types(i)  = p_real_dp
          END DO
       ELSE
    ! In this case, we "wrap around" the largest longitude.
          dt2_size = nglat/2 + 2*(nglat - nglat/2)
          DO i=0, nglat - nglat/2 - 1
             dt2_blklen(nglat/2 + 1 + 2*i) = nlon - glons(2) + 1
             dt2_displs(nglat/2 + 1 + 2*i) = (glons(2) - 1)*sizeofdp   &
                      + (nglat/2 + i)*fs_size(1)*fs_size(2)*sizeofdp
             dt2_types(nglat/2 + 1 + 2*i)  = p_real_dp
             dt2_blklen(nglat/2 + 2 + 2*i) = glone(2)
             dt2_displs(nglat/2 + 2 + 2*i) =                           &
                        (nglat/2 + i)*fs_size(1)*fs_size(2)*sizeofdp
             dt2_types(nglat/2 + 2 + 2*i)  = p_real_dp
          END DO
       END IF
    ! Now we can fit the puzzle together.
       CALL MPI_Type_create_struct(dt2_size,                           & 
                                   dt2_blklen, dt2_displs, dt2_types,  &
                                   fs_col_raw, perror)
    ! From this point on we proceed in precise analogy to the handling
    ! of the data type gpx_base on the sender side.
    ! Next we have to adjust the extent. We want to handle this
    ! type as if it were a single column of fs.
    ! This is where the "silent transpose" happens.
       CALL MPI_Type_get_extent(fs_col_raw, lb_fsc, sizeoffsc, perror)
     
       CALL MPI_Type_create_resized(fs_col_raw, lb_fsc, &
                                    fs_size(1)*sizeofdp, &
                                    fs_col, perror)
    ! Put the various copies of fs_col together. To show the
    ! analogy, the same code pattern is used as for gpx_base.
       CALL MPI_Get_address(fs(1,1,1,1), fs_base, perror)
       CALL MPI_Get_address(fs(1,1,1,1), dt_displs(1), perror)
       CALL MPI_Get_address(fs(1,1,1,2), dt_displs(2), perror)
       CALL MPI_Get_address(fs(1,1,1,3), dt_displs(3), perror)
       CALL MPI_Get_address(fs(1,1,1,4), dt_displs(4), perror)
       CALL MPI_Get_address(fs(1,1,1,5), dt_displs(5), perror)
       CALL MPI_Get_address(fs(1,1,1,6), dt_displs(6), perror)
       CALL MPI_Get_address(fs(1,1,1,7), dt_displs(7), perror)
       IF (ke == nlev + 1) THEN
           lexcess_level = .TRUE.
           nk = nk - 1
           ke = ke - 1
       ...
       DO i=1,7
          dt_blklen(i) = nk
          dt_displs(i) = dt_displs(i) - fs_base
          dt_types(i)  = fs_col
       END DO
       IF ( lexcess_level .AND. PRESENT(sf1) ) dt_blklen(1) = dt_blklen(1) + 1
       IF ( lexcess_level .AND. PRESENT(sf2) ) dt_blklen(2) = dt_blklen(2) + 1
       IF ( lexcess_level .AND. PRESENT(sf3) ) dt_blklen(3) = dt_blklen(3) + 1
       CALL MPI_Type_create_struct(nvar, dt_blklen, dt_displs, dt_types,  &
                                   all2all_types(k,recv_3d), perror)
    ! Commit the result
       CALL MPI_Type_commit(all2all_types(k,recv_3d), perror)

    ! step 7 * MPI data types for receiving 2d fields
    !
    ! This data stream just contains one contiguous block of real numbers
    ! with block size nk0*nglat*3
       CALL MPI_Type_contiguous( nk0*nglat*3, p_real_dp, &
                                 all2all_types(k,recv_2d), perror)
    ! Commit the result
       CALL MPI_Type_commit(all2all_types(k,recv_2d), perror)

    END DO

    all2all_types_initialized(1) = .TRUE.

    END SUBROUTINE setup_gp_fs_parms
!------------------------------------------------------------------------------
  END SUBROUTINE tr_gp_fs_alt
#endif
!==============================================================================
#ifndef NOMPI
  SUBROUTINE tr_fs_gp_alt (gl_dc, gp1, gp2, gp3, gp4, gp5, gp6, gp7, gp8, &
                           gp9, sf1, sf2, sf3, zm1, zm2, zm3, fs, fs0)
    !
    ! transpose grid point space <-  Fourier space
    !
    !
    ...
    REAL(dp)           ,INTENT(inout)  :: gp6    (:,:,:)   !
    REAL(dp)           ,INTENT(inout)  :: gp7    (:,:,:)   !
    REAL(dp)           ,INTENT(inout)  :: gp8    (:,:,:)   ! for u wind deriv.
    REAL(dp)           ,INTENT(inout)  :: gp9    (:,:,:)   ! for v wind deriv.
    REAL(dp) ,OPTIONAL ,INTENT(inout)  :: sf1    (:,:)     ! gridpoint space 2d
    REAL(dp) ,OPTIONAL ,INTENT(inout)  :: sf2    (:,:)     ! gridpoint space 2d
    REAL(dp) ,OPTIONAL ,INTENT(inout)  :: sf3    (:,:)     ! gridpoint space 2d
    REAL(dp)           ,INTENT(inout)  :: zm1    (:,:)     ! zonal mean
    ...
    ! local variables
    !
    INTEGER, PARAMETER :: send_3d   = 4
    INTEGER, PARAMETER :: recv_3d   = 5
    INTEGER, PARAMETER :: send_2d   = 6
    INTEGER, PARAMETER :: recv_2d   = 7
    !
!TMP    INTEGER :: i, j, k, l, n
    INTEGER :: i, j, k
    INTEGER :: imype         ! index of this pe
    ...
    ! They are the same for all calls
    IF (.NOT. ALLOCATED(all2all_types) .OR. &
        .NOT. all2all_types_initialized(2) ) THEN
       CALL setup_fs_gp_parms
    END IF

    ! If everything is set up, do the communication
    CALL MPI_Alltoallw(                                                       &
         fs,  counts(:,send_3d), displs(:,send_3d), all2all_types(:,send_3d), &
         gp1, counts(:,recv_3d), displs(:,recv_3d), all2all_types(:,recv_3d), &
         p_communicator_a, perror)
    CALL MPI_Alltoallw(                                                       &
         fs0, counts(:,send_2d), displs(:,send_2d), all2all_types(:,send_2d), &
         zm1, counts(:,recv_2d), displs(:,recv_2d), all2all_types(:,recv_2d), &
         p_communicator_a, perror)

  CONTAINS

    SUBROUTINE setup_fs_gp_parms
    ...
    LOGICAL                      :: lexcess_level
    INTEGER                      :: dt_size
    INTEGER                      :: dt_blklen(12), dt_types(12)
    INTEGER(KIND=p_address_kind) :: dt_displs(12)
    INTEGER                      :: dt2_size
    INTEGER                      :: dt2_blklen(gl_dc(imype)%nglat * 2)
    INTEGER                      :: dt2_types (gl_dc(imype)%nglat * 2)
    INTEGER(KIND=p_address_kind) :: dt2_displs(gl_dc(imype)%nglat * 2)
    ...
    ! step 4 * MPI data types for receiving 3d fields
    !
    ! Each of the fields gp1(:,:,:), ... , gp9(:,:,:) is of 
    ! shape (nproma,nlev,ngpblks). The portion to be sent to another
    ! MPI task is gpx(:,ks:ke,:), 
    ! but only nglat*nglon*(ke-ks+1) p_real_dp numbers are to be received.
    ! To achieve this, we use the fact that
    ...
    ! Now piece the various gpx_base fields together
       ...
       CALL MPI_Get_address(gp6(1,ks,1), dt_displs(j+2), perror)
       CALL MPI_Get_address(gp7(1,ks,1), dt_displs(j+3), perror)
       CALL MPI_Get_address(gp8(1,ks,1), dt_displs(j+4), perror)
       CALL MPI_Get_address(gp9(1,ks,1), dt_displs(j+5), perror)
       dt_size = j + 5
       DO i=j, dt_size
          dt_blklen(i) = nk
          dt_displs(i) = dt_displs(i) - gp1_base
    ...
    ! step 5 * MPI data types for receiving 2d fields
    !
    ! This data stream just contains contiguous blocks of real numbers
    ! with block size nk0.
       CALL MPI_Type_contiguous( nk0, p_real_dp, zm_base_raw, perror)
    ...
    ! step 6 * MPI data types for sending 3d fields
    !
    ! For the MPI data type on the receiver side, the roles of imype
    ! and gl_dc_inx(k) have to be swapped.
    ...
    ! This is being complicated by <some range> varying over 1..nglat.
    !
    ! By definition in mo_decomposition.f90, nglh = nglat/2.
    !
    ! For 1..nglat/2, the range is given by glons(1)..glone(1)
       DO i=1,nglat/2
          dt2_blklen(i) = glone(1) - glons(1) + 1
    ...
    ! analogy, the same code pattern is used as for gpx_base.
       ...
       CALL MPI_Get_address(fs(1,1,1,6), dt_displs(6), perror)
       CALL MPI_Get_address(fs(1,1,1,7), dt_displs(7), perror)
       CALL MPI_Get_address(fs(1,1,1,8), dt_displs(8), perror)
       CALL MPI_Get_address(fs(1,1,1,9), dt_displs(9), perror)
       IF (ke == nlev + 1) THEN
           lexcess_level = .TRUE.
           nk = nk - 1
           ke = ke - 1
    ...
    ! step 7 * MPI data types for sending 2d fields
    !
    ! This data stream just contains one contiguous block of real numbers
    ! with block size nk0*nglat*3
       CALL MPI_Type_contiguous( nk0*nglat*3, p_real_dp, &
    ...
    all2all_types_initialized(2) = .TRUE.

    END SUBROUTINE setup_fs_gp_parms
!------------------------------------------------------------------------------
  END SUBROUTINE tr_fs_gp_alt
#endif
!==============================================================================
