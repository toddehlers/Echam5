...
USE mo_wiso,           ONLY : fractcal, tnat, nwisotyp                 &
                            , talphal1, talphal2, talphal3             &
                            , talphas1, talphas2, talphas3             &
                            , tsatbase, tsatfac, tdifrel, calc_delta   &
                            , cwisomin, cwisosec
!---wiso-code-end
...
!---wiso-code
  ...
  REAL(dp):: pwisoqm1(kbdim,klev,kwiso)                                &
           , pwisoxlm1(kbdim,klev,kwiso)                               &
           , pwisoxim1(kbdim,klev,kwiso), pwisoxtec(kbdim,klev,kwiso)  &
           , pwisoqtec(kbdim,klev,kwiso)
  REAL(dp):: pwisoxlvi(kbdim,kwiso),      pwisoxivi(kbdim,kwiso)
  REAL(dp):: pwisoaprl(kbdim,kwiso)                                    &
           , pwisoqvi(kbdim,kwiso),       pwisossfl(kbdim,kwiso)
  REAL(dp):: pwisoqte(kbdim,klev,kwiso)
  REAL(dp):: pwisoxlte(kbdim,klev,kwiso), pwisoxite(kbdim,klev,kwiso)
  REAL(dp):: pwisoaprs(kbdim,kwiso),      pwisorsfl(kbdim,kwiso)
...
!---wiso-code
  ...
  REAL(dp):: zwisosnmlt, zwisoximelt                                   &
           ...
           , zwisozdrr, zwisozdrs                                      &
           , zwisoqp1, zwisoxlp1, zwisoqold, zwisoxlold, zwisoxiold    &
           , zwisodqcor, zwisodxicor, zwisodxlcor                      &
           , zwisoxlfrac, zwisoxifrac                                  &
...
!       Caculate fractionation factors for isotopic changes during evaporation of rain
        CALL fractcal(kproma,kbdim,kwiso,zwisofracliq,zwisofracice,ptm1(:,jk),tmelt)
...
!---wiso-code-end
           ...
           lo4_wiso(jl)= (zqcdif.GT.zqsec*zqp1)                                !---wiso-code: remember water source for zqcdif
           zqcdif     = MIN(zqcdif,zqsec*zqp1)             ! limit to qv       !              (true=zqp1)
!
...
!
        ELSE !lcover=.false. or jk < ncctop
           ...
           zqcdif_tmp(jl) = (zqvdt-zdqsat)                                     !---wiso-code: store value of zqcdif
           lo3_wiso(jl)= (zqcdif.GT.(-zxilb*zclcaux(jl)))                      !---wiso-code: remember water source of zqcdif
           zqcdif         = MAX(zqcdif,-zxilb*zclcaux(jl))                     !               (true=zqcdif, false=zxilb)
           lo4_wiso(jl)= (zqcdif.GT.zqsec*zqp1)                                !---wiso-code: remember water source for zqcdif
           zqcdif         = MIN(zqcdif,zqsec*zqp1)                             !              (true=zqp1)
           lo_zqcdif(jl) = (zqcdif.LT.0.0_dp)                                  !---wiso-code: remember if zqcdif is negative
           IF (zqcdif .LT. 0.0_dp) THEN                 ! cloud dissipation
              zifrac      = zxib(jl)/MAX(zepsec,zxilb)
              zifrac      = MAX(MIN(zifrac,1.0_dp),0.0_dp)
...
!       Caculate fractionation factors for condensation/deposition and evaporation/sublimation
        CALL fractcal(kproma,kbdim,kwiso,zwisofracliq,zwisofracice,ztp1_tmp(:),tmelt)
...
!           detrained cloud water/ice and ice sedimentation - water isotopes
            ...
            IF (lo2_new(jl)) THEN                                         ! ice cloud
              ...
              IF (locc(jl)) THEN                                   ! cloud cover > 0.
                zwisoxib(jl,jt)     = pwisoxim1(jl,jk,jt)/zclcaux(jl)
                zwisoxlb(jl,jt)     = pwisoxlm1(jl,jk,jt)/zclcaux(jl)
                ...
                ELSE
                  ...
                  zwisoxib(jl,jt)   = zwisoxib(jl,jt)+MAX(zwisoxidt/zclcaux(jl),           &
                                      -zwisoxib(jl,jt))
                ...
                ELSE
                  ...
                  zwisoxlb(jl,jt)   = zwisoxlb(jl,jt)+MAX(zwisoxldt/zclcaux(jl),           &
                                      -zwisoxlb(jl,jt))
...
!             (explicit in-line calculation to enable code vectorization)
              ...
              zwisofracliq(jl,jt) = exp(talphal1(jt)/(ztp1_tmp2(jl)**2._dp)+talphal2(jt)/ztp1_tmp2(jl)+talphal3(jt)) ! frac. liquid
              zwisofracice(jl,jt) = exp(talphas1(jt)/(ztp1_tmp2(jl)**2._dp)+talphas2(jt)/ztp1_tmp2(jl)+talphas3(jt)) ! frac. ice

              IF (nwisotyp(jt).ne.1) THEN        ! effective fractionation over ice if necessary
                IF (ztp1_tmp2(jl).lt.tmelt) THEN
              ...
              IF (lo_zqcdif(jl)) THEN                                          ! cloud dissipation
                ...
                zwisodenom=MERGE(zqp1_tmp2(jl),zwisodenom,lo4_wiso(jl))        !source: zqp1
                zwisonumer=MERGE(zwisoqp1,zwisonumer,lo4_wiso(jl))
              ...
              ELSE                                                             ! cloud generation
                ...
                IF (lo2_new(jl)) THEN            ! deposition to frozen cloud ice (fractionation as open system)
                  ...
                  zwisodenom=zxlb_tmp2(jl)+zxib_tmp2(jl)                                     ! source: zxib
                  zwisonumer=zwisoxlb(jl,jt)+zwisoxib(jl,jt)
                  zwisodenom=MERGE(zqcdif_tmp(jl),zwisodenom,lo3_wiso(jl))     ! source: zqcdif
                  zwisonumer=MERGE(zwisoqcdif,zwisonumer,lo3_wiso(jl))
                  zwisodenom=MERGE(zqp1_tmp2(jl), zwisodenom,lo4_wiso(jl))     ! source: zqp1
            ...
            ELSE                                                              !lcover=.false. or jk < ncctop
              ...
              IF (lo_zqcdif(jl)) THEN                                         ! cloud dissipation (zqcdif .LT. 0.)
                ...
                zwisodenom=MERGE(zqcdif_tmp(jl),zwisodenom,lo3_wiso(jl))       !source: zqcdif
                zwisonumer=MERGE(zwisoqcdif,zwisonumer,lo3_wiso(jl))
                zwisodenom=MERGE(zqp1_tmp(jl),zwisodenom,lo4_wiso(jl))         !source: zqp1
                zwisonumer=MERGE(zwisoqp1,zwisonumer,lo4_wiso(jl))
              ...
              ELSE                                                             ! cloud generation
                ...
                IF (lo2_new(jl)) THEN            ! deposition to frozen cloud ice (fractionation as open system)
                  ...
                  zwisodenom=zxlb_tmp(jl)+zxib_tmp(jl)                                     ! source: zxib
                  zwisonumer=zwisoxlb(jl,jt)+zwisoxib(jl,jt)
                  zwisodenom=MERGE(zqcdif_tmp(jl),zwisodenom,lo3_wiso(jl))     ! source: zqcdif
                  zwisonumer=MERGE(zwisoqcdif,zwisonumer,lo3_wiso(jl))
                  zwisodenom=MERGE(zqp1_tmp(jl),zwisodenom,lo4_wiso(jl))       ! source: zqp1
...
!           (explicit in-line calculation to enable code vectorization)
            ...
            zwisofracliq(jl,jt) = exp(talphal1(jt)/(ztp1tmp_tmp(jl)**2._dp)+talphal2(jt)/ztp1tmp_tmp(jl)+talphal3(jt)) ! frac. liquid
            zwisofracice(jl,jt) = exp(talphas1(jt)/(ztp1tmp_tmp(jl)**2._dp)+talphas2(jt)/ztp1tmp_tmp(jl)+talphas3(jt)) ! frac. ice
    
            IF (nwisotyp(jt).ne.1) THEN        ! effective fractionation over ice if necessary
              IF (ztp1tmp_tmp(jl).lt.tmelt) THEN
