...
!---wiso-code
  ...
  USE mo_memory_g3b,         ONLY: sn, wl, ws, wsmx, wisosn, wisowl, wisows
  USE mo_decomposition,      ONLY: ldc => local_decomposition
  USE mo_clim,               ONLY: tslclim
  USE mo_constants,          ONLY: tmelt
  USE mo_wiso,               ONLY: nwiso, nwisotyp, twisoatm           &
                                 , twisosur1, twisosur2, tnat,nisw
...
! comment out the following default Echam5 code to use zin for water isotope initialization
!  IF (p_pe == p_io) DEALLOCATE (zin)
...
!---wiso-code
  ...
  ! initialize 3D water isotope fields
  IF (nwiso > 0) THEN
     IF (p_pe == p_io) THEN
        ! info%dim variables are still set from reading in Q grid point data
        ALLOCATE (zin2(info%gdim(1), info%gdim(2), nwiso, info%gdim(3)))  
        zin2(:,:,:,:) = 0._dp
        ! initialize atmospheric water isotope fields. vapour fields
        DO jt=1,nwiso      
           IF (nwisotyp(jt).eq.1) THEN      
              ! if tracer is h2-16o
              ! wiso(jt) = q
              zin2(:,:,jt,:) = zin(1:info%gdim(1),:,:) 
           ELSE
              ! if tracer is not h2-16o
              ! wiso(jt) = nat. isotope-ratio * humidity q * 
              ! (1 + initial deviation from smow in atmosphere)
              zin2(:,:,jt,:) = tnat(jt)*zin(1:info%gdim(1),:,:)*(1._dp+twisoatm(jt))   
           ENDIF
        ENDDO
     ENDIF
       
     CALL scatter_gp (zin2, wisoq(:,:,1:nwiso,:), global_decomposition)
      
     IF (p_pe == p_io) DEALLOCATE (zin2)
      
! cloud water and cloud ice set initial to zero - water isotopes
  ...
  IF ((.NOT.lvctch) .AND. p_pe == p_io) DEALLOCATE (zin)
...
! read in water isotope ocean surface fields
  IF (nwiso > 0) THEN
     ...
     IF (p_pe == p_io) THEN
        ...
        CALL get_stream_element_info (g3b, 'wisosw_d', info)

        ALLOCATE (zin(info%gdim(1), info%gdim(3), info%gdim(2)))
        ALLOCATE (zin3(info%gdim(1), info%gdim(2), info%gdim(3)))
        ...
        DO jt = 1, nwiso
           zin3(:,jt,:) = zin(1:info%gdim(1),1:info%gdim(3),jt) 
        END DO

     ENDIF
       
     CALL get_stream_element (g3b, 'wisosw_d', zptr)

     CALL scatter_gp (zin3, zptr, global_decomposition)

     IF (p_pe == p_io)  DEALLOCATE (zin)     
     IF (p_pe == p_io)  DEALLOCATE (zin3)     
...
! calculate annual mean temperature from climate land surface temperatures
     ...
     DO jmonth = 1, 12
       DO jg = 1, ldc%ngpblks
         DO jl=1,ldc%nproma 
             tsurface(jl,jg) = tsurface(jl,jg) + tslclim(jl,jg,jmonth) 
         END DO
       END DO
     END DO
     ...
     DO jg = 1, ldc%ngpblks
       DO jl=1,ldc%nproma 
           tsurface(jl,jg) = tsurface(jl,jg)/12._dp
       END DO
     END DO
    ...
    DO jg=1,ldc%ngpblks 
       DO jt=1,nwiso
         DO jl=1,ldc%nproma 
           ...
           zwisosurf(jl,jt,jg)=twisosur1(jt)*(tsurface(jl,jg) - tmelt) - twisosur2(jt)
           IF (nwisotyp(jt).eq.2) THEN  ! if tracer = 18o: zwisosurf = min (-4./1000., zxtatm)
             zwisosurf(jl,jt,jg)=min( -4._dp/1000._dp,zwisosurf(jl,jt,jg))
           ELSE IF (nwisotyp(jt).eq.3) THEN  ! if tracer = hdo: zxtatm = min (-22./1000., zxtatm)
             zwisosurf(jl,jt,jg)=min(-22._dp/1000._dp,zwisosurf(jl,jt,jg))
           ENDIF
         END DO
       END DO
     END DO
     ...
     DO jg=1,ldc%ngpblks
       DO jt=1,nwiso
         DO jl=1,ldc%nproma
           IF (nwisotyp(jt).eq.1) THEN
             ! if tracer = h2-16o: set tracer reservoirs = evap. mask * corresponding wetness reservoir
             wisosn(jl,jt,jg)=sn(jl,jg)
             wisowl(jl,jt,jg)=wl(jl,jg)
             wisows(jl,jt,jg)=ws(jl,jg)
           ELSE
             ! if tracer not h2-16o:
             ! set tracer reservoirs = evap. mask * nat.isotope-ratio * corresponding wetness reservoir * (1.+zxtatm)
             wisosn(jl,jt,jg)=tnat(jt)*sn(jl,jg)*(1._dp+zwisosurf(jl,jt,jg))
             wisowl(jl,jt,jg)=tnat(jt)*wl(jl,jg)*(1._dp+zwisosurf(jl,jt,jg))
             wisows(jl,jt,jg)=tnat(jt)*ws(jl,jg)*(1._dp+zwisosurf(jl,jt,jg))
           ENDIF
           wisows(jl,jt,jg) = MIN(wisows(jl,jt,jg),wsmx(jl,jg)) ! maximum ws value for any tracer: wsmx (isotope-independent)
         END DO
       END DO
     END DO
